...
@test "extract_hosts: basic IP extraction from sshd failure" {
	local input="Feb 22 10:15:03 myhost sshd[12345]: Failed password for root from 203.0.113.100 port 22 ssh2"
	local result
...
@test "extract_hosts: multiple patterns match different failure types" {
	local input
	input=$(printf '%s\n' \
		"Feb 22 10:15:03 myhost sshd[12345]: Invalid user admin from 192.0.2.5 port 54321 ssh2" \
		"Feb 22 10:15:05 myhost sshd[12345]: Failed password for root from 203.0.113.100 port 22 ssh2")
	local result
	...
	[ "$count" -eq 2 ]
	echo "$result" | grep -qF "192.0.2.5"
	echo "$result" | grep -qF "203.0.113.100"
}

@test "extract_hosts: strips ::ffff: prefix" {
	local input="Feb 22 10:15:03 myhost sshd[12345]: Failed password for root from ::ffff:192.0.2.1 port 22 ssh2"
	local result
	result=$(echo "$input" | extract_hosts "sshd.*Failed password for .* from <HOST>")
	[ "$result" = "192.0.2.1" ]
}
...
@test "extract_hosts: strips brackets around IP" {
	local input="Feb 22 10:15:03 myhost postfix/smtpd[12345]: warning: [203.0.113.50]: SASL PLAIN authentication failed"
	local result
...
@test "extract_hosts: no matches returns nothing" {
	local input="Feb 22 10:15:03 myhost sshd[12345]: Accepted publickey for user from 192.0.2.1 port 22 ssh2"
	local result
	result=$(echo "$input" | extract_hosts "sshd.*Failed password for .* from <HOST>")
	[ -z "$result" ]
}
...
@test "extract_hosts: IGNOREREGEX excludes matching lines" {
	...
	local input
	input=$(printf '%s\n' \
		"Feb 22 myhost dovecot: pop3-login: Aborted login (auth failed, 1 attempts): user=<admin>, rip=203.0.113.100, lip=192.0.2.1" \
...
@test "extract_hosts: multiple IPs from multiple lines" {
	...
	input=$(printf '%s\n' \
		...
		"Feb 22 myhost sshd[2]: Failed password for root from 192.0.2.2 port 22 ssh2" \
		"Feb 22 myhost sshd[3]: Failed password for root from 192.0.2.3 port 22 ssh2")
	local result
	result=$(echo "$input" | extract_hosts "sshd.*Failed password for .* from <HOST>")
	local count
	count=$(echo "$result" | wc -l)
...
@test "extract_hosts: duplicate IPs preserved for counting" {
	local input
	input=$(printf '%s\n' \
		"Feb 22 myhost sshd[1]: Failed password for root from 203.0.113.1 port 22 ssh2" \
		"Feb 22 myhost sshd[2]: Failed password for admin from 203.0.113.1 port 22 ssh2" \
		"Feb 22 myhost sshd[3]: Failed password for user from 203.0.113.1 port 22 ssh2")
	local result
	result=$(echo "$input" | extract_hosts "sshd.*Failed password for .* from <HOST>")
	local count
	count=$(echo "$result" | wc -l)
...
@test "extract_hosts: IP in brackets (postfix style)" {
	local input="Feb 22 myhost postfix/smtpd[9876]: warning: unknown[198.51.100.40]: SASL LOGIN authentication failed: authentication failure"
	local result
	result=$(echo "$input" | extract_hosts "\[<HOST>\].*SASL.*authentication failed")
...
@test "extract_hosts: IP after rip= (dovecot style)" {
	local input="Feb 22 myhost dovecot: imap-login: Aborted login (auth failed, 1 attempts): user=<test>, method=PLAIN, rip=198.51.100.50, lip=192.0.2.1"
	local result
...
@test "extract_hosts: ::ffff: prefix still strips to IPv4" {
	local input="Feb 22 10:15:03 myhost sshd[12345]: Failed password for root from ::ffff:203.0.113.1 port 22 ssh2"
	local result
	result=$(echo "$input" | extract_hosts "sshd.*Failed password for .* from <HOST>")
	[ "$result" = "203.0.113.1" ]
}
