...
@test "state_bans_active_remove: preserves other entries after remove" {
	...
	state_bans_active_append "$INSTALL_PATH" "1001" "0" "192.0.2.2" "dovecot" "143"
	state_bans_active_append "$INSTALL_PATH" "1002" "0" "192.0.2.3" "postfix" "25"
	state_bans_active_remove "$INSTALL_PATH" "192.0.2.2"
	# 192.0.2.1 and 192.0.2.3 must still be present
	run state_bans_active_check "$INSTALL_PATH" "192.0.2.1"
	assert_success
	run state_bans_active_check "$INSTALL_PATH" "192.0.2.3"
	assert_success
	# 192.0.2.2 must be gone
...
@test "state_events_prune: respects cutoff window" {
	...
	state_events_append "$INSTALL_PATH" "100" "192.0.2.1" "sshd" "3"
	state_events_append "$INSTALL_PATH" "900" "192.0.2.2" "sshd" "2"
	...
	[ "$count" -eq 2 ]
	# only 192.0.2.2 events should remain
	run cat "$INSTALL_PATH/tmp/events.dat"
