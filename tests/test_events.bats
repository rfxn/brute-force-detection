...
@test "state_events_append: single event appended" {
	state_events_append "$INSTALL_PATH" "1708560000" "10.0.0.1" "sshd"
	run cat "$INSTALL_PATH/tmp/events.dat"
	assert_output "1708560000 10.0.0.1 sshd"
}

@test "state_events_append: bulk append with count" {
	state_events_append "$INSTALL_PATH" "1708560000" "10.0.0.1" "sshd" "3"
	local line_count
...
@test "state_events_append: format is TIMESTAMP IP MOD" {
	state_events_append "$INSTALL_PATH" "1708560000" "192.168.1.100" "dovecot"
	run cat "$INSTALL_PATH/tmp/events.dat"
	assert_output "1708560000 192.168.1.100 dovecot"
}

@test "state_events_append: multiple appends accumulate" {
	state_events_append "$INSTALL_PATH" "1708560000" "10.0.0.1" "sshd" "2"
	state_events_append "$INSTALL_PATH" "1708560001" "10.0.0.2" "dovecot" "1"
	local line_count
	line_count=$(wc -l < "$INSTALL_PATH/tmp/events.dat")
	[ "$line_count" -eq 3 ]
}
...
@test "state_events_append: count=0 appends nothing" {
	state_events_append "$INSTALL_PATH" "1708560000" "10.0.0.1" "sshd" "0"
	local line_count
	line_count=$(wc -l < "$INSTALL_PATH/tmp/events.dat")
	[ "$line_count" -eq 0 ]
}
...
@test "state_events_count: returns 0 for empty file" {
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "1708560300"
	assert_output "0"
}

@test "state_events_count: counts events within window" {
	state_events_append "$INSTALL_PATH" "1708560100" "10.0.0.1" "sshd" "3"
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "1708560300" "sshd"
	assert_output "3"
}

@test "state_events_count: excludes events outside window" {
	# event at t=100, window=300, now=500 => cutoff=200 => event at 100 excluded
	state_events_append "$INSTALL_PATH" "100" "10.0.0.1" "sshd" "2"
	# event at t=300, within window
	state_events_append "$INSTALL_PATH" "300" "10.0.0.1" "sshd" "1"
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "500" "sshd"
	assert_output "1"
}

@test "state_events_count: per-service filter" {
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "sshd" "3"
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "dovecot" "2"
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "1200" "sshd"
	assert_output "3"
}
...
@test "state_events_count: cross-service (no mod filter)" {
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "sshd" "3"
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "dovecot" "2"
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "1200"
	assert_output "5"
}

@test "state_events_count: does not match partial IPs" {
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "sshd" "5"
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.10" "sshd" "3"
	run state_events_count "$INSTALL_PATH" "10.0.0.1" "300" "1200" "sshd"
	assert_output "5"
}
...
@test "state_events_count: returns 0 for unknown host" {
	state_events_append "$INSTALL_PATH" "1000" "10.0.0.1" "sshd" "5"
	run state_events_count "$INSTALL_PATH" "10.0.0.99" "300" "1200" "sshd"
	assert_output "0"
}
...
@test "state_events_prune: removes old events" {
	...
	state_events_append "$INSTALL_PATH" "100" "10.0.0.1" "sshd" "2"
	# recent event
	state_events_append "$INSTALL_PATH" "900" "10.0.0.2" "sshd" "1"
...
@test "state_events_prune: safety cap enforced" {
	...
	for i in $(seq 1 10); do
		state_events_append "$INSTALL_PATH" "900" "10.0.0.$i" "sshd"
	done
...
@test "count_failures: zero for absent host" {
	local hosts_parsed
	hosts_parsed=$(printf "10.0.0.1\n10.0.0.2\n")
	run count_failures "10.0.0.3" "$hosts_parsed" "$INSTALL_PATH" "300" "1000" "sshd"
	assert_output "0"
}

@test "count_failures: windowed expiry" {
	# pre-seed old events (will be outside window)
	state_events_append "$INSTALL_PATH" "500" "10.0.0.1" "sshd" "5"
...
@test "count_failures: per-service isolation" {
	...
	state_events_append "$INSTALL_PATH" "900" "10.0.0.1" "dovecot" "5"
	local hosts_parsed
	hosts_parsed=$(printf "10.0.0.1\n10.0.0.1\n")
