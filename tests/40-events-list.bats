...
# --- _apool_awk sort/limit extensions ---

@test "_apool_awk: default sort_mode=count, limit=25 (backward compat)" {
	local pool="$INSTALL_PATH/stats/attack.pool"
	local now
	now=$(date +"%s")
...
@test "_apool_awk: sort_mode=time sorts by last_seen descending" {
	local pool="$INSTALL_PATH/stats/attack.pool"
	local now
	now=$(date +"%s")
...
@test "_apool_awk: sort_mode=ip sorts by IP ascending" {
	local pool="$INSTALL_PATH/stats/attack.pool"
	local now
	now=$(date +"%s")
...
@test "_apool_awk: limit=0 returns all entries" {
	local pool="$INSTALL_PATH/stats/attack.pool"
	local now
	now=$(date +"%s")
	local i
...
@test "_apool_awk: limit=25 truncates to 25 entries (default)" {
	local pool="$INSTALL_PATH/stats/attack.pool"
	local now
	now=$(date +"%s")
	local i
...
@test "events_list: shows IPs from attack.pool sorted by count" {
	_seed_pool
...
@test "events_list: header row present" {
	_seed_pool
	run events_list "$INSTALL_PATH"
	assert_success
...
@test "events_list: shows country code" {
	_seed_pool
	run events_list "$INSTALL_PATH"
	assert_success
...
@test "events_list: multiple services shown comma-separated" {
	_seed_pool
	run events_list "$INSTALL_PATH"
	assert_success
	# .10 has sshd + dovecot
...
@test "events_list_ip: shows per-service breakdown" {
	_seed_pool
	run events_list_ip "$INSTALL_PATH" "192.0.2.10"
	assert_success
...
@test "events_list_ip: shows ban triggers count" {
	_seed_pool
	run events_list_ip "$INSTALL_PATH" "192.0.2.10"
	assert_success
...
@test "events_list_ip: shows country" {
	_seed_pool
	run events_list_ip "$INSTALL_PATH" "192.0.2.10"
	assert_success
	assert_output --partial "Country:"
...
@test "events_list_ip: shows first/last seen" {
	_seed_pool
	run events_list_ip "$INSTALL_PATH" "192.0.2.10"
	assert_success
...
@test "events_list_ip: no events shows message" {
	run events_list_ip "$INSTALL_PATH" "192.0.2.99"
	assert_success
	assert_output --partial "No events for"
}

@test "events_list_ip: invalid IP returns error" {
	run events_list_ip "$INSTALL_PATH" "not-an-ip"
	assert_failure
	assert_output --partial "error:"
}

@test "events_list_ip: shows live pressure when available" {
	_seed_pool
	local now
	now=$(date +"%s")
...
@test "events_list_cidr: finds IPs in /24 range" {
	_seed_pool
...
@test "events_list_cidr: shows summary line with totals" {
	_seed_pool
	run events_list_cidr "$INSTALL_PATH" "192.0.2.0/24"
	assert_success
