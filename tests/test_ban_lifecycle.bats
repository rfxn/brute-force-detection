...
@test "state_bans_active_append: single entry appended" {
	...
	assert_output "1000 1300 10.0.0.1 sshd 22"
}

@test "state_bans_active_append: duplicate host skipped" {
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.1" "sshd" "22"
	state_bans_active_append "$INSTALL_PATH" "2000" "2300" "10.0.0.1" "dovecot" "110"
	local count
...
@test "state_bans_active_append: different hosts both added" {
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.1" "sshd" "22"
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.2" "sshd" "22"
	local count
	count=$(wc -l < "$INSTALL_PATH/tmp/bans.active")
	[ "$count" -eq 2 ]
}
...
@test "state_bans_active_append: format is TIMESTAMP EXPIRY IP MOD PORTS" {
	state_bans_active_append "$INSTALL_PATH" "1708560000" "1708560300" "192.168.1.100" "sshd" "22"
	run cat "$INSTALL_PATH/tmp/bans.active"
	assert_output "1708560000 1708560300 192.168.1.100 sshd 22"
}
...
@test "state_bans_active_remove: removes entry" {
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.1" "sshd" "22"
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.2" "sshd" "22"
	state_bans_active_remove "$INSTALL_PATH" "10.0.0.1"
	run cat "$INSTALL_PATH/tmp/bans.active"
	assert_output "1000 1300 10.0.0.2 sshd 22"
}

@test "state_bans_active_remove: preserves other entries" {
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.1" "sshd" "22"
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.2" "dovecot" "110"
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.3" "postfix" "25"
	state_bans_active_remove "$INSTALL_PATH" "10.0.0.2"
	local count
	count=$(wc -l < "$INSTALL_PATH/tmp/bans.active")
	[ "$count" -eq 2 ]
	run grep -c "10.0.0.2" "$INSTALL_PATH/tmp/bans.active"
	assert_output "0"
}

@test "state_bans_active_remove: handles missing host gracefully" {
	state_bans_active_append "$INSTALL_PATH" "1000" "1300" "10.0.0.1" "sshd" "22"
	run state_bans_active_remove "$INSTALL_PATH" "10.0.0.99"
	assert_success
}
...
@test "state_bans_active_list: permanent ban shows permanent" {
	state_bans_active_append "$INSTALL_PATH" "1708560000" "0" "10.0.0.5" "dovecot" "110,995"
	run state_bans_active_list "$INSTALL_PATH"
	assert_success
...
@test "state_bans_active_expired: mixed entries returns only expired" {
	echo "1000 1300 10.0.0.1 sshd 22" >> "$INSTALL_PATH/tmp/bans.active"
	echo "1000 0 10.0.0.2 dovecot 110" >> "$INSTALL_PATH/tmp/bans.active"
	echo "1000 2000 10.0.0.3 postfix 25" >> "$INSTALL_PATH/tmp/bans.active"
	# at now=1500: 10.0.0.1 (expiry=1300) is expired, 10.0.0.2 (permanent) is not, 10.0.0.3 (expiry=2000) is not
	run state_bans_active_expired "$INSTALL_PATH" "1500"
	assert_output --partial "10.0.0.1"
	refute_output --partial "10.0.0.2"
	refute_output --partial "10.0.0.3"
}
...
@test "state_bans_history_append: appends correctly" {
	...
	assert_output "1000 1300 10.0.0.1 sshd ban"
}

@test "state_bans_history_append: format is TIMESTAMP EXPIRY IP MOD ACTION" {
	state_bans_history_append "$INSTALL_PATH" "1708560000" "1708560300" "192.168.1.100" "sshd" "ban"
	state_bans_history_append "$INSTALL_PATH" "1708560300" "1708560300" "192.168.1.100" "sshd" "unban"
	local count
...
@test "state_bans_count_recent: counts bans within window" {
	...
	state_bans_history_append "$INSTALL_PATH" "950" "1250" "10.0.0.1" "sshd" "ban"
	# now=1000, window=200, cutoff=800
	run state_bans_count_recent "$INSTALL_PATH" "10.0.0.1" "200" "1000"
	assert_output "2"
}

@test "state_bans_count_recent: excludes bans outside window" {
	state_bans_history_append "$INSTALL_PATH" "100" "400" "10.0.0.1" "sshd" "ban"
	state_bans_history_append "$INSTALL_PATH" "900" "1200" "10.0.0.1" "sshd" "ban"
...
@test "state_bans_count_recent: counts only ban and escalate actions" {
	state_bans_history_append "$INSTALL_PATH" "900" "1200" "10.0.0.1" "sshd" "ban"
	state_bans_history_append "$INSTALL_PATH" "950" "0" "10.0.0.1" "sshd" "escalate"
	state_bans_history_append "$INSTALL_PATH" "960" "1200" "10.0.0.1" "sshd" "unban"
...
@test "state_bans_count_recent: returns 0 for unknown host" {
	state_bans_history_append "$INSTALL_PATH" "900" "1200" "10.0.0.1" "sshd" "ban"
	run state_bans_count_recent "$INSTALL_PATH" "10.0.0.99" "200" "1000"
	assert_output "0"
}
