...
# ============================================================
# _geoip_cidr4_to_ranges — CIDR to integer-range conversion
...
@test "geoip_cidr4_to_ranges: /24 produces correct range" {
	...
	run _geoip_cidr4_to_ranges "$TEST_TMPDIR/test.zone" "AU"
	assert_success
	assert_output "16777216 16777471 AU"
}

@test "geoip_cidr4_to_ranges: /32 produces single-IP range" {
	_load_cidr4_to_ranges
	# 10.0.0.1/32 = 167772161 to 167772161 (1 IP)
	echo "10.0.0.1/32" > "$TEST_TMPDIR/test.zone"
	run _geoip_cidr4_to_ranges "$TEST_TMPDIR/test.zone" "XX"
	assert_success
	assert_output "167772161 167772161 XX"
}

@test "geoip_cidr4_to_ranges: /8 produces correct large range" {
	_load_cidr4_to_ranges
...
@test "geoip_cidr4_to_ranges: /0 produces full IP space" {
	_load_cidr4_to_ranges
...
@test "geoip_cidr4_to_ranges: /1 produces half IP space" {
	_load_cidr4_to_ranges
...
@test "geoip_cidr4_to_ranges: /16 produces correct range" {
	_load_cidr4_to_ranges
...
@test "geoip_cidr4_to_ranges: multiple CIDRs produce multiple lines" {
	_load_cidr4_to_ranges
...
@test "geoip_cidr4_to_ranges: skips comment and blank lines" {
	_load_cidr4_to_ranges
	cat > "$TEST_TMPDIR/test.zone" << 'EOF'
# This is a comment
1.0.0.0/24
...
@test "geoip_cidr4_to_ranges: known IP 223.255.254.0/24 falls in expected range" {
	_load_cidr4_to_ranges
	# 223.255.254.0/24 → start = 3758095872, end = 3758096127
	echo "223.255.254.0/24" > "$TEST_TMPDIR/test.zone"
	run _geoip_cidr4_to_ranges "$TEST_TMPDIR/test.zone" "CN"
	assert_success
...
# ============================================================
# geoip_all_cc — country code enumeration
# (now in geoip_lib.sh, previously _all_cc_codes in update-ipcountry.sh)
# ============================================================
...
@test "geoip_all_cc: returns all known country codes" {
	_load_all_cc_codes
	run geoip_all_cc
	assert_success
...
@test "geoip_all_cc: includes known countries from each continent" {
	_load_all_cc_codes
	run geoip_all_cc
	assert_success
...
@test "ip_to_country: reads CIDR-generated integer-range format" {
	...
	{
		_geoip_cidr4_to_ranges "$TEST_TMPDIR/xx.zone" "XX"
		_geoip_cidr4_to_ranges "$TEST_TMPDIR/ru.zone" "RU"
